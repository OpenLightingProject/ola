...
void ClientTest::testGetSetDMX() {
  ...
  OLA_ASSERT_DMX_EQUALS(source2.Data(), buffer);
  ...
  OLA_ASSERT_DMX_EQUALS(source2.Data(), old_data);
  OLA_ASSERT_EQ(timestamp, source2.Timestamp());
  OLA_ASSERT_EQ((uint8_t) 100, source2.Priority());
  ...
  OLA_ASSERT_DMX_EQUALS(buffer, source3.Data());
  ...
  OLA_ASSERT_DMX_EQUALS(empty, source4.Data());
}

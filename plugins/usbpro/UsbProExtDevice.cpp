...
UsbProExtDevice::UsbProExtDevice(ola::PluginAdaptor *plugin_adaptor,
  ...
  m_widget->GetParameters(NewSingleCallback(
    this,
    &UsbProExtDevice::UpdateParams));

  // device_id determines i/o count, max 16

//  uint8_t input_port_count  = (uint8_t) ((device_id >> 8) & 0x000F);
  uint8_t output_port_count = (uint8_t)  (device_id & 0x000F);

  UsbProExtInputPort *input_port = new UsbProExtInputPort(
      this,
      m_widget,
      0,
      plugin_adaptor,
  ...
  for (unsigned int id=0; id <= output_port_count; id++) {
	  OutputPort *output_port = new UsbProExtOutputPort(
		  this,
		  m_widget,
		  id,
		  str.str(),
 ...
 * Stop this device
 */
void UsbProExtDevice::PrePortStop() {
  m_widget->Stop();
}
 ...
 */
void UsbProExtDevice::Configure(RpcController *controller,
 ...
 */
void UsbProExtDevice::UpdateParams(bool status,
 ...
 * client.
 */
void UsbProExtDevice::HandleParametersRequest(RpcController *controller,
  ...
  if (request->has_parameters() &&
    ...
    bool ret = m_widget->SetParameters(
      request->parameters().has_break_time() ?
  ...
  m_widget->GetParameters(NewSingleCallback(
    this,
    &UsbProExtDevice::HandleParametersResponse,
    controller,
    response,
    done));
}
 ...
 */
void UsbProExtDevice::HandleParametersResponse(
 ...
 */
void UsbProExtDevice::HandleSerialRequest(
    RpcController *controller,
    const Request *request,
    string *response,
    ConfigureCallback *done) {

...
/*
 * Create a new RenardThread object
 */
RenardThread::RenardThread(const string &path)
    : ola::thread::Thread(),
    m_fd(INVALID_FD),
    m_path(path),
    m_term(false) {
 ...
 *
 */
bool RenardThread::WriteDmx(const DmxBuffer &buffer) {
  MutexLocker locker(&m_mutex);

 ...
 */
NewSLPStore::ReturnCode NewSLPStore::Insert(const TimeStamp &now,
 ...
 */
NewSLPStore::ReturnCode NewSLPStore::Remove(const ServiceEntry &entry) {
 ...
 *   skipped.
 */
bool NewSLPStore::BulkInsert(const TimeStamp &now,
 ...
 */
void NewSLPStore::Lookup(const TimeStamp &now,
                         set<string> &scopes,
  ...
  for (unsigned int i = 0; service_iter != iter->second->entries.end();
       i++, ++service_iter) {
    if (!service_iter->WithScopes(scopes))
      continue;
    if (limit && i >= limit)
      break;
 ...
 * @param now the current time
 */
void NewSLPStore::Clean(const TimeStamp &now) {
 ...
 */
void NewSLPStore::Dump(const TimeStamp &now) {
  ServiceMap::iterator iter = m_services.begin();
 ...
 */
void NewSLPStore::MaybeCleanURLList(const TimeStamp &now,
 ...
 */
NewSLPStore::ServiceMap::iterator NewSLPStore::Populate(const TimeStamp &now,
                                                        const string &service) {
 ...
 * @returns Either OK or SCOPE_MISMATCH.
 */
NewSLPStore::ReturnCode NewSLPStore::InsertOrUpdateEntry(

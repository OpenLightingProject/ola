 ...
 *
 * NewSLPStoreTest.cpp
 * Test fixture for the NewSLPStore class
...
#include "tools/slp/NewSLPStore.h"

using ola::MockClock;
using ola::TimeStamp;
using ola::slp::NewSLPStore;
...
class NewSLPStoreTest: public CppUnit::TestFixture {
  CPPUNIT_TEST_SUITE(NewSLPStoreTest);
...
const char NewSLPStoreTest::SCOPE1[] = "scope1";
const char NewSLPStoreTest::SCOPE2[] = "scope2";
const char NewSLPStoreTest::SCOPE3[] = "scope3";
const char NewSLPStoreTest::SERVICE1[] = "one";
const char NewSLPStoreTest::SERVICE2[] = "two";
const char NewSLPStoreTest::SERVICE1_URL1[] = "service:one://192.168.1.1";
const char NewSLPStoreTest::SERVICE1_URL2[] = "service:one://192.168.1.2";
const char NewSLPStoreTest::SERVICE2_URL1[] = "service:two://192.168.1.1";
const char NewSLPStoreTest::SERVICE2_URL2[] = "service:two://192.168.1.3";
...
CPPUNIT_TEST_SUITE_REGISTRATION(NewSLPStoreTest);

/*
 * Check that we can insert and lookup entries.
 */
void NewSLPStoreTest::testInsertAndLookup() {
  ...
  OLA_ASSERT_EQ(NewSLPStore::OK, m_store.Insert(now, service1));
  m_store.Lookup(now, test_scopes, SERVICE1, &services);
  ...
  OLA_ASSERT_EQ(NewSLPStore::OK, m_store.Insert(now, service2));
  m_store.Lookup(now, test_scopes, SERVICE1, &services);
  ...
  OLA_ASSERT_EQ(NewSLPStore::OK, m_store.Insert(now, service3));
 ...
 */
void NewSLPStoreTest::testDoubleInsert() {
  ServiceEntries services, expected_services;
  m_store.Lookup(now, test_scopes, SERVICE1, &services);
  ...
  OLA_ASSERT_EQ(NewSLPStore::OK, m_store.Insert(now, service));
  ...
  services.clear();
  OLA_ASSERT_EQ(NewSLPStore::OK, m_store.Insert(now, service_shorter));
  m_store.Lookup(now, test_scopes, SERVICE1, &services);
  OLA_ASSERT_URLENTRIES_EQ(expected_services, services);

  // now insert the longer one
  services.clear();
  OLA_ASSERT_EQ(NewSLPStore::OK, m_store.Insert(now, service_longer));
  expected_services.clear();
  ...
  OLA_ASSERT_EQ(NewSLPStore::SCOPE_MISMATCH,
 ...
 */
void NewSLPStoreTest::testBulkInsert() {
 ...
 * Test Remove()
 */
void NewSLPStoreTest::testRemove() {
  ServiceEntries services, expected_services;
  ServiceEntry service1(test_scopes, SERVICE1_URL1, 10);
  OLA_ASSERT_EQ(NewSLPStore::OK, m_store.Insert(now, service1));
  ...
  OLA_ASSERT_EQ(NewSLPStore::OK, m_store.Remove(service1));
 ...
 * Test aging.
 */
void NewSLPStoreTest::testAging() {
  ServiceEntry service(test_scopes, SERVICE1_URL1, 10);

...
#ifndef TOOLS_SLP_NEWSLPSTORE_H_
#define TOOLS_SLP_NEWSLPSTORE_H_
...
/**
class ScopeSetFactory {
  public:
    ScopeSetFactory(const set<string> &scopes)

    ScopeSet* Create(const set<string> &scopes);

  private:
    // map string to byte offset
};
...
class ScopeSet {
  public:
    ...
    bool operator==(const ScopeSet &other) {
      return m_vector == other.m_vector;
    }

    bool Contains(const ScopeSet &other) {
      return m_vector && other.m_vector;
    }
...
/**
 * Represents an SLP Service
 */
class ServiceEntry {
  public:
    ServiceEntry() : m_lifetime(0) {}
    ...
    bool WithScopes(const set<string> &scopes) const {
    ...
    friend ostream& operator<<(ostream &out, const ServiceEntry &entry) {
...
#endif  // TOOLS_SLP_NEWSLPSTORE_H_

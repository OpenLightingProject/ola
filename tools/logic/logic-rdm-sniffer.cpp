...
DEFINE_a_bool(display_asc, false,
              "Display non-RDM alternate start code frames");
DEFINE_d_bool(display_dmx, false, "Display DMX Frames");
DEFINE_s_bool(full_rdm, true, "Display the full RDM frame");
DEFINE_t_bool(timestamp, false, "Include timestamps");

DEFINE_uint16(dmx_slot_limit, DMX_UNIVERSE_SIZE,
              "Only display the first N DMX slots");


void OnReadData(uint64_t device_id, uint8_t *data, uint32_t data_length,
                void *user_data) {
...
void OnError(uint64_t device_id, void *user_data) {
...
class LogicReader {
  public:
    ...
    void DeviceConnected(uint64_t device, GenericInterface *interface);
    void DeviceDisconnected(uint64_t device);

    void Stop();

  private:
    uint64_t m_device_id;  // GUARDED_BY(mu_);
...
void LogicReader::DeviceConnected(uint64_t device,
...
void LogicReader::DeviceDisconnected(uint64_t device) {
...
void OnConnect(uint64_t device_id, GenericInterface* device_interface,
               void* user_data) {
  if (!user_data)
    return;

  LogicReader *reader = dynamic_cast<LogicReader*>(user_data);  // NOLINT
  reader->DeviceConnected(device_id, device_interface);
}

void OnDisconnect(uint64_t device_id, void* user_data) {
  if (!user_data)
    return;

...
class CircularDepdendencyException(Error):
  """Raised if there is a circular depdendency created by PROVIDES &
     REQUIRES statements.
  """
  ...
  def _AddTest(self, device, class_name_to_object, deps_map, test_class,
    ...
    for dep_class in dep_classes:
      if dep_class in new_parents:
        raise CircularDepdendencyException(
            'Circular depdendency found %s in %s' % (dep_class, new_parents))

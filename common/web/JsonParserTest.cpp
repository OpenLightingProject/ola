...
void JsonParserTest::testParseBool() {
  string error;
  auto_ptr<JsonValue> value(JsonParser::Parse("  true  ", &error));
...
void JsonParserTest::testParseNull() {
  string error;
  auto_ptr<JsonValue> value(JsonParser::Parse("  null  ", &error));
  OLA_ASSERT_NOT_NULL(value.get());
...
void JsonParserTest::testParseString() {
  string error;
  auto_ptr<JsonValue> value(JsonParser::Parse("\"test\"", &error));
  OLA_ASSERT_NOT_NULL(value.get());
...
void JsonParserTest::testParseNumber() {
  string error;
  auto_ptr<JsonValue> value(JsonParser::Parse(" 0", &error));
  OLA_ASSERT_NOT_NULL(value.get());
...
void JsonParserTest::testArray() {
  string error;
  auto_ptr<JsonValue> value(JsonParser::Parse(" [  ]", &error));
  OLA_ASSERT_NOT_NULL(value.get());
  ...
  value.reset(JsonParser::Parse("[\"test]\", 1, [\"nested\"], 4] ", &error));
  OLA_ASSERT_NOT_NULL(value.get());
  OLA_ASSERT_EQ(string("[\"test]\", 1, [\"nested\"], 4]"),
                JsonWriter::AsString(*value.get()));
...
void JsonParserTest::testObject() {
  ...
  // Multiple keys
  value.reset(JsonParser::Parse("{\"key1\"  : 1, \"key2\"  : 2} ", &error));
  OLA_ASSERT_NOT_NULL(value.get());
  OLA_ASSERT_EQ(string("{\n  \"key1\": 1,\n  \"key2\": 2\n}"),
                JsonWriter::AsString(*value.get()));
...
void JsonParserTest::testStressTests() {
  ...
  */
  ...
  value.reset(JsonParser::Parse("{ \"v\" : 1.7976931348623157E308}", &error));
  OLA_ASSERT_NOT_NULL(value.get());
  OLA_ASSERT_EQ(string("{\n  \"v\": 1.79769e+308\n}"),
                JsonWriter::AsString(*value.get()));
